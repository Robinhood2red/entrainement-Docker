...
//
// Fonctionnalités :
// - Consultation des détails d'une séance (date, durée, notes)
// - Gestion des exercices (CRUD)
// - Formulaire dynamique selon la catégorie (Cardio vs Musculation/Flexibilité)
// - Sélecteur d'exercice via modal imbriquée
// ============================================================================
...
export default function WorkoutDetailScreen() {
  ...
  // ? Requêtage : chargement parallèle de la séance et du catalogue d'exercices ?
  const loadWorkout = async () => {
    try {
      ...
      // * Action : hydratation des états de la séance et des exercices disponibles *
  ...
  // * Action : préparation et ouverture du formulaire d'ajout *
  const openAddEx = () => {
  ...
  // * Action : pré-remplissage du formulaire pour édition d'un exercice *
  ...
  // ! Sécurité / Alerte : confirmation avant retrait d'un exercice de la séance !
  ...
  // ? Requêtage : suppression de l'exercice associé à la séance via l'API ?
  const handleDeleteEx = async (weId: number) => {
    try {
      await api.delete(`/workouts/${workoutId}/exercises/${weId}`)
      // * Action : mise à jour optimiste locale de la liste des exercices *
      setWorkout((prev) =>
  ...
  // ? Requêtage / Logique : création ou mise à jour de l'exercice dans la séance ?
  const handleSubmitEx = async () => {
    ...
    try {
      // * Action : préparation du payload nettoyé *
      const payload = {
  ...
  // * Action / Logique métier : détermination du type d'exercice pour l'UI dynamique *
  ...
  return (
    <View style={styles.root}>
      <ScrollView
        ...
        refreshControl={
          // ? Requêtage : rafraîchissement manuel de la séance ?
          <RefreshControl
                ...
                <View key={we.id} style={styles.exCard}>
                  <View style={styles.exHeader}>
                    ...
                    <View style={styles.exActions}>
                      ...
                      {/* ! Sécurité : déclenchement de la suppression ! */}
      ...
      >
        <View style={styles.modal}>
          ...
          <ScrollView style={styles.modalBody} keyboardShouldPersistTaps="handled">
            ...
            {/* * Action / Logique UI : bascule des champs du formulaire selon le type d'exercice * */}
            {isCardio ? (
              <>
        ...
        {/* ── Modal 2 : Sélecteur d'exercice imbriqué ── */}
        <Modal
            ...
            <ScrollView>
              {allExercises.map((ex) => (
                <TouchableOpacity
                  ...
                  onPress={() => {
                    // * Action : sélection de l'exercice et réinitialisation du formulaire *

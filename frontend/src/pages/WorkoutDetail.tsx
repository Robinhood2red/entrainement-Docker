// ====================================================================================
// pages/WorkoutDetail.tsx — Gestion détaillée d'une séance d'entraînement.
// Ce script gère l'affichage complet d'un workout, le chargement initial en parallèle,
// l'ajout d'exercices ainsi que l'édition et la suppression inline par exercice.
// ====================================================================================
...
export default function WorkoutDetail() {
  // ? Récupération de l'ID de la séance depuis l'URL dynamique ?
  ...
  // * États locaux pour l'isolation de l'édition inline par exercice *
  ...
  // ? Chargement parallèle des données de la séance et de la liste de référence des exercices ?
  useEffect(() => {
    if (!id) return
    Promise.all([
      api.get(`/workouts/${id}`),
      ...
      .then(([wRes, eRes]) => {
        // * Hydratation des états suite à la récupération des données *
        setWorkout(wRes.data.workout)
        setAllExercises(eRes.data.exercises)
        setAddForm((f) => ({ ...f, exercise_id: eRes.data.exercises[0]?.id ?? 0 }))
      })
      .catch(() => { 
        // ! Alerte : redirection immédiate si la séance demandée est introuvable !
  ...
  // * Passage d'un exercice spécifique en mode édition (remplissage du formulaire inline) *
  ...
  // * Annulation du mode édition pour nettoyer l'état local *
  ...
  // * Mutation de l'état d'édition inline au fil de la saisie utilisateur *
  const changeEdit = (weId: number, field: keyof EditState, value: string) => {
    setEditing((prev) => ({ ...prev, [weId]: { ...prev[weId], [field]: value } }))
  }

  // ? Envoi des modifications de l'exercice à l'API (PATCH) ?
  const saveEdit = async (weId: number) => {
    ...
    try {
      ...
      // * Mise à jour de l'affichage global avec la nouvelle structure retournée *
      setWorkout(res.data.workout)
      cancelEdit(weId)
      toast.success('Exercice mis à jour')
    } catch {
      // ! Alerte : échec de la sauvegarde de la ligne !
  ...
  // ? Suppression d'un exercice lié à la séance (DELETE) ?
  const confirmDelete = async (weId: number) => {
    if (!id) return
    setDeleting(weId)
    try {
      const res = await api.delete(`/workouts/${id}/exercises/${weId}`)
      // * Actualisation de la liste des exercices du workout *
      setWorkout(res.data.workout)
      toast.success('Exercice retiré')
    } catch {
      // ! Alerte : échec de la suppression sur le serveur !
  ...
  // * Déduction de la catégorie pour adapter dynamiquement l'interface d'ajout *
  ...
  // ? Envoi du nouvel exercice pour liaison avec la séance (POST) ?
  const submitAdd = async () => {
    ...
    try {
      ...
      // * Réinitialisation du formulaire et fermeture du volet d'ajout *
      setWorkout(res.data.workout)
      setAddForm((f) => ({ ...f, sets: '', reps: '', weight_used: '', duration: '' }))
      setAddOpen(false)
      toast.success('Exercice ajouté')
    } catch {
      // ! Alerte : échec de l'association de l'exercice !
  ...
  // ! Sécurité / UX : Affichage du spinner global tant que le Promise.all n'est pas résolu !
      ...
      <div className="bg-[#1E293B] border border-slate-700/50 rounded-2xl p-6">
        ...
        {exercises.length === 0 ? (
          ...
          <div className="divide-y divide-slate-700/50">
            {exercises.map((ex) => {
              // * Détermination de l'état d'édition et de chargement pour la ligne courante *

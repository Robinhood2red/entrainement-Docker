// ==============================================================================================
// pages/Exercises.tsx — Gestion des exercices (CRUD complet avec patterns avancés React)
//
// Ce script gère l'affichage, la recherche debouncée, le filtrage par catégorie ainsi que la
// création, modification et suppression optimiste d'exercices via une modale réutilisable.
// ==============================================================================================
...
export default function Exercises() {
  ...
  const loadExercises = (s = search, c = catFilter) => {
    // ? Initialisation des paramètres de requêtage pour l'API ?
    const params: Record<string, string> = {}
    if (c) params.category = c
    if (s) params.search = s
    
    // ? Récupération des exercices filtrés depuis le backend ?
    api
      .get('/exercises', { params })
      .then((res) => setExercises(res.data.exercises))
      // ! Alerte : échec critique de récupération des données à afficher !
  ...
  // * Action : Ouverture de la modal en mode création et reset du formulaire *
  const openCreate = () => {
    setEditTarget(null)
    setForm(EMPTY_FORM)
    setModalOpen(true)
  ...
  // * Action : Ouverture de la modal en mode édition et pré-remplissage des champs *
  ...
  // * Logique métier : Mise à jour dynamique de l'état du formulaire (clé-valeur) *
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      if (editTarget) {
        // * Logique métier : Soumission de la mise à jour (PUT) et modification optimiste locale *
        ...
        // * Logique métier : Soumission de la création (POST) et insertion en tête de liste *
      ...
      // ! Alerte : Erreur lors du traitement ou du transfert des données du formulaire !
      const msg =
  ...
  const handleDelete = async (id: number) => {
    try {
      // * Action : Requête de suppression de la ressource cible *
      await api.delete(`/exercises/${id}`)
      // * Logique métier : Filtrage optimiste du tableau local pour fluidifier l'UI *
    ...
    <div className="space-y-5">
      ...
      {deleteId !== null && (
        <Modal onClose={() => setDeleteId(null)} title="Supprimer l'exercice">
          {/* ! Alerte : Avertissement de suppression définitive d'une ressource ! */}
